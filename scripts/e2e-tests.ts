...
async function createGzWithJson(data: unknown): Promise<Uint8Array> {
  const jsonBuffer = Buffer.from(JSON.stringify(data));
  const compressed = await gzipAsync(jsonBuffer);
  return new Uint8Array(compressed);
}

async function createTgzWithJson(data: unknown, filename = "data.json"): Promise<Uint8Array> {
  const pack = tar.pack();
...
async function suiteCronAutoBackup(): Promise<void> {
  ...
  await test("GIVEN no auth WHEN calling POST /api/cron/trigger THEN returns 401", async () => {
    const res = await fetch(`${baseUrl}/api/cron/trigger`, { method: "POST" });
    assertEqual(res.status, 401, "status");
  });

  await test("GIVEN wrong token WHEN calling POST /api/cron/trigger THEN returns 401", async () => {
    const res = await fetch(`${baseUrl}/api/cron/trigger`, {
      method: "POST",
  ...
  await test("GIVEN a successful trigger WHEN listing cron logs THEN has success entry for backy-test", async () => {
    ...
    const res = await fetch(`${baseUrl}/api/cron/logs?projectId=${PROJECT_ID}&status=success`);
    assertEqual(res.status, 200, "status");
    const body = await res.json();
  ...
  await test("GIVEN cron logs WHEN listing with pageSize=1 THEN returns paginated results", async () => {
    const res = await fetch(`${baseUrl}/api/cron/logs?pageSize=1`);
    assertEqual(res.status, 200, "status");
    const body = await res.json();
  ...
  await test("GIVEN cron logs WHEN deleting by projectId THEN returns 204", async () => {
    const res = await fetch(`${baseUrl}/api/cron/logs?projectId=${PROJECT_ID}`, {
      method: "DELETE",
    });
  ...
  await test("GIVEN auto-backup with custom header WHEN triggering THEN project triggers with header", async () => {
    ...
    const updateRes = await fetch(`${baseUrl}/api/projects/${PROJECT_ID}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  await test("GIVEN remaining cron logs WHEN deleting all THEN returns 204", async () => {
    const res = await fetch(`${baseUrl}/api/cron/logs`, { method: "DELETE" });
    assertEqual(res.status, 204, "status");
  });
...
async function suiteManualUpload(): Promise<void> {
  ...
  await test("GIVEN a .txt file WHEN uploading manually THEN returns 201 (all formats accepted)", async () => {
    ...
    const body = await res.json();
    assert(typeof body.id === "string" && body.id.length > 0, "id should be a non-empty string");
    createdBackupIds.push(body.id);
  });
}

async function suiteHappyPathGz(): Promise<void> {
  ...
  await test("GIVEN a .json.gz file WHEN uploading via webhook THEN returns 201", async () => {
    ...
    createdBackupIds.push(webhookBackupId);
  });
  ...
  await test("GIVEN a GZ backup WHEN querying metadata THEN file_type=gz and is_single_json=0", async () => {
    const res = await fetch(`${baseUrl}/api/backups/${webhookBackupId}`);
    assertEqual(res.status, 200, "status");
    const body = await res.json();
  ...
  await test("GIVEN a GZ backup without extraction WHEN requesting preview THEN returns 404", async () => {
    const res = await fetch(`${baseUrl}/api/backups/${webhookBackupId}/preview`);
    assertEqual(res.status, 404, "status");
  });
  ...
  await test("GIVEN a GZ backup WHEN downloading THEN returns valid gzip data", async () => {
    const dlRes = await fetch(`${baseUrl}/api/backups/${webhookBackupId}/download`);
    const { url } = await dlRes.json();
  ...
  await test("GIVEN a .json.gz file WHEN uploading via manual upload THEN returns 201", async () => {
    ...
    createdBackupIds.push(manualBackupId);
  });
  ...
  await test("GIVEN a manually uploaded GZ WHEN querying metadata THEN file_type=gz", async () => {
    const res = await fetch(`${baseUrl}/api/backups/${manualBackupId}`);
    assertEqual(res.status, 200, "status");
    const body = await res.json();
    assertEqual(body.file_type, "gz", "file_type");
    assertEqual(body.sender_ip, "manual-upload", "sender_ip");
  ...
  await test("GIVEN a GZ backup WHEN restoring via public endpoint THEN returns valid download URL", async () => {
    const res = await fetch(`${baseUrl}/api/restore/${webhookBackupId}?token=${WEBHOOK_TOKEN}`);
    assertEqual(res.status, 200, "status");
    const body = await res.json();
    assert(typeof body.url === "string", "url should be a string");
  });
...
async function suiteHappyPathTgz(): Promise<void> {
...
async function suiteUnknownFileType(): Promise<void> {
