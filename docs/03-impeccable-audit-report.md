...
- 修复说明：已新增 `info`、`warning`、`surface-elevated` 语义 token，并替换登录页、loading、JSON viewer、上传图标和 cron 状态中的硬编码颜色；其余颜色映射后续继续收口。
...
- 修复说明：已为项目、备份活动和 cron 图表补充底部文本摘要/统计块，降低对 hover tooltip 的依赖；完整表格 fallback 暂未引入。
...
- 修复说明：已改为请求 `/api/backups?pageSize=5`，避免首页先拉默认分页结果再在客户端截断。
...
- 修复说明：已将 webhook/prompt 区块和 recent backups 区块拆分为独立组件，先降低主页面 JSX 复杂度；其余设置区块后续继续拆分。
...
- 修复说明：已为面包屑导航补充 `aria-label="Breadcrumb"`，并为当前项增加 `aria-current="page"`。
...
- 修复说明：已为 backups 和 cron logs 页面内容容器补充明确的 `relative` 定位边界，稳定后续加载遮罩范围。
